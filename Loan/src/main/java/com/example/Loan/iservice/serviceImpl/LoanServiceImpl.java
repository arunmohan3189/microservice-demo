...
import com.example.Loan.ObjectMapper.LoansMapper;
...
public class LoanServiceImpl  implements ILoanService{
	...
	if(loan.isPresent()) {
		throw new LoanAlreadyExitException("Loan Already Exit for given mobile number = "+mobileNumber);
	}
	loanRepository.save(createLoans(mobileNumber));	
	...
	public LoansDto fetchLoan(String mobileNumber) {
		...
		Loans loan = optionalLoan.orElseThrow(() ->new ResourceNotFoundException("Loan", "Mobile Number", mobileNumber));
	...
	public boolean updateLoanDetails(LoansDto loansDto) {
		...
		Optional<Loans> optLoan = loanRepository.findByMobileNumber(mobileNumber);
		Loans loan = optLoan.orElseThrow(() -> new ResourceNotFoundException("Loan", "Mobile Number", mobileNumber));
